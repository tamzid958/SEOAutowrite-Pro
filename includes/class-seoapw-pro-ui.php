...
class SEOAPW_Pro_UI {
	...
	public static function enqueue_assets( string $hook ): void {
		...
		wp_localize_script( 'seoapw-pro', 'seoapwPro', array(
			...
			'strings'          => array(
				'activating'   => __( 'Activating…', 'seoautowrite-pro' ),
				'deactivating' => __( 'Deactivating…', 'seoautowrite-pro' ),
				'activated'    => __( 'Pro Active', 'seoautowrite-pro' ),
				'error'        => __( 'An error occurred. Please try again.', 'seoautowrite-pro' ),
				'confirmDeact' => __( 'Deactivate your Pro license on this site?', 'seoautowrite-pro' ),
			),
		) );
	}
	...
	public static function render_balance_indicator(): void {
		...
		if ( $balance <= 0 ) {
			...
			$label = sprintf(
				...
				__( 'Balance: %s · Running on Ollama', 'seoautowrite-pro' ),
				'$0.00'
			);
		} elseif ( $low ) {
			...
			$label = sprintf(
				...
				__( 'Balance: %1$s · %2$d article remaining', 'seoautowrite-pro' ),
			...
			$label = sprintf(
				/* translators: 1: balance, 2: article count */
				__( 'Balance: %1$s · %2$d articles remaining', 'seoautowrite-pro' ),
				'$' . number_format( $balance, 2 ),
				$articles
			);
		}
		...
		<div class="<?php echo esc_attr( $class ); ?>">
			...
			<a href="<?php echo esc_url( $top_up ); ?>" target="_blank" class="seoapw-balance__topup">
				<?php echo $low || $balance <= 0 ? esc_html__( 'Top Up Now', 'seoautowrite-pro' ) : esc_html__( 'Top Up', 'seoautowrite-pro' ); ?>
			</a>
		</div>
		<?php
	}
		...
		switch ( $type ) {
			case 'zero_balance':
				return sprintf(
					...
					__( '🔴 <strong>SEOAutowrite Pro:</strong> Your balance is $0.00. This article was generated by Ollama. <a href="%s" target="_blank">Top Up Now</a> to resume SEOAutowrite AI generation.', 'seoautowrite-pro' ),
					$top_up_url
				);
			...
			case 'low_balance_post':
				...
				return sprintf(
					...
					__( '⚠️ <strong>SEOAutowrite Pro:</strong> Pro article published. Balance is low (%1$s remaining). <a href="%2$s" target="_blank">Top Up Now</a>', 'seoautowrite-pro' ),
					esc_html( $balance ),
					$top_up_url
				);
			...
			case 'auto_topup':
				...
				return sprintf(
					...
					__( '💳 <strong>SEOAutowrite Pro:</strong> Balance was low. %1$s auto top-up charged to your saved payment method. New balance: %2$s.', 'seoautowrite-pro' ),
					esc_html( $amount ),
					esc_html( $balance )
				);
		}
	...
	public static function render_license_card(): void {
		...
		<section class="seoapw-card seoapw-license-card">
			<h2 class="seoapw-card-title"><?php esc_html_e( 'Pro License', 'seoautowrite-pro' ); ?></h2>

			<?php if ( $is_pro ) : ?>

				<!-- Status: Active -->
				<div class="seoapw-row">
					<div class="seoapw-label"><?php esc_html_e( 'Status', 'seoautowrite-pro' ); ?></div>
					<div class="seoapw-control">
						<span class="seoapw-badge seoapw-badge--active">
							<?php esc_html_e( '✅ Pro Active', 'seoautowrite-pro' ); ?>
						</span>
				...
				<!-- Balance -->
				<div class="seoapw-row">
					<div class="seoapw-label"><?php esc_html_e( 'Balance', 'seoautowrite-pro' ); ?></div>
					<div class="seoapw-control">
						<strong>$<?php echo esc_html( number_format( $balance, 2 ) ); ?></strong>
						<span class="seoapw-articles-rem">
							&mdash; <?php echo esc_html( sprintf( _n( '%d article remaining', '%d articles remaining', $articles_rem, 'seoautowrite-pro' ), $articles_rem ) ); ?>
						</span>
						<a href="<?php echo esc_url( $top_up_url ); ?>" target="_blank" class="seoapw-btn seoapw-btn-topup" style="margin-left:12px;">
							<?php esc_html_e( 'Top Up Balance', 'seoautowrite-pro' ); ?>
						</a>
					</div>
				</div>
				...
				<!-- Auto top-up -->
				<div class="seoapw-row">
					<div class="seoapw-label"><?php esc_html_e( 'Auto Top-Up', 'seoautowrite-pro' ); ?></div>
					<div class="seoapw-control">
						<?php if ( $auto_topup ) : ?>
							<span class="seoapw-badge seoapw-badge--active">
								<?php
						...
						<?php else : ?>
							<span class="seoapw-badge seoapw-badge--inactive"><?php esc_html_e( 'Disabled', 'seoautowrite-pro' ); ?></span>
						<?php endif; ?>
						<a href="<?php echo esc_url( defined( 'SEOAPW_LS_PORTAL_URL' ) ? SEOAPW_LS_PORTAL_URL : '#' ); ?>"
						   target="_blank" class="seoapw-manage-billing-link">
							<?php esc_html_e( 'Manage Billing', 'seoautowrite-pro' ); ?>
						</a>
					</div>
				</div>
				...
				<div class="seoapw-row">
					...
					<div class="seoapw-control">
						<button type="button" id="seoapw-deactivate-btn" class="seoapw-btn-link seoapw-btn-deactivate">
							<?php esc_html_e( 'Deactivate license on this site', 'seoautowrite-pro' ); ?>
						</button>
			...
			<?php else : ?>
				...
				<div class="seoapw-row">
					...
					<div class="seoapw-control">
						<span class="seoapw-badge seoapw-badge--inactive">
							<?php esc_html_e( '⚠️ No Active License', 'seoautowrite-pro' ); ?>
						</span>
					</div>
				</div>
				...
				<div class="seoapw-row">
					<label class="seoapw-label" for="seoapw-license-key-input">
						<?php esc_html_e( 'License Key', 'seoautowrite-pro' ); ?>
					</label>
					<div class="seoapw-control">
						<div class="seoapw-model-row">
							...
							<button type="button" id="seoapw-activate-btn" class="seoapw-btn">
								<?php esc_html_e( 'Activate', 'seoautowrite-pro' ); ?>
							</button>
						</div>
						<span id="seoapw-activate-status" class="seoapw-btn-status"></span>
						<p class="seoapw-desc">
							<?php esc_html_e( 'Enter your Lemon Squeezy license key to enable Pro features.', 'seoautowrite-pro' ); ?>
						</p>
					</div>
				</div>
	...
	public static function ajax_activate_license(): void {
		...
		if ( empty( $key ) ) {
			wp_send_json_error( array( 'message' => __( 'Please enter a license key.', 'seoautowrite-pro' ) ) );
		}
		...
		if ( empty( $result['valid'] ) ) {
			$reason = ! empty( $result['reason'] ) ? $result['reason'] : __( 'invalid_key', 'seoautowrite-pro' );
			wp_send_json_error( array(
				'message' => sprintf(
					/* translators: %s: error reason */
					__( '❌ Invalid Key (%s)', 'seoautowrite-pro' ),
					esc_html( $reason )
				),
			) );
		}
		...
		wp_send_json_success( array(
			'message'      => __( '✅ Pro Active', 'seoautowrite-pro' ),
	...
	public static function ajax_deactivate_license(): void {
		...
		wp_send_json_success( array(
			'message' => __( 'License deactivated. The plugin will now use Ollama for generation.', 'seoautowrite-pro' ),
		) );
	}
